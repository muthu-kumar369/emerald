...
const CountSection = ({data}) => {
    return (
        <section className="count-section">
            {data?.bgImage && <div className={`background-image ${data?.quote?.text ? "py-24":""}`} style={{ backgroundImage: `url(${data?.bgImage?.src})`, height:`${data?.bgImage?.height}px` }}>
                <div className="overlay"></div>
                {data?.quote && <div className='inline text-white text-4xl text-center'>
                    <p className='quote xl:mx-64'>{data?.quote?.text}</p>
                    </div>}
